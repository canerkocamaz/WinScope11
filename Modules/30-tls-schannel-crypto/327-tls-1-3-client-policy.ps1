...
[pscustomobject]@{
    Id                = 327
    Name              = 'TLS 1.3 Client Policy'
    Group             = 'TLS, Schannel & Crypto'
    GroupId           = 30
    Flags             = '[R][N]'
    Description       = 'Reports Schannel TLS 1.3 client protocol policy.'
    Adapter           = 'Native'
    Risk              = 'ReportOnly'
    ...
    Invoke            = {
        param($Context)
        $checks = @(
            @{Item='TLS 1.3 Client';Path='HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\TLS 1.3\Client';Names=@('Enabled','DisabledByDefault')}
        )
