...
[pscustomobject]@{
    Id                = 325
    Name              = 'TLS 1.2 Client Policy'
    Group             = 'TLS, Schannel & Crypto'
    GroupId           = 30
    Flags             = '[R][N]'
    Description       = 'Reports Schannel TLS 1.2 client protocol policy.'
    Adapter           = 'Native'
    Risk              = 'ReportOnly'
    ...
    Invoke            = {
        param($Context)
        $checks = @(
            @{Item='TLS 1.2 Client';Path='HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\TLS 1.2\Client';Names=@('Enabled','DisabledByDefault')}
        )
