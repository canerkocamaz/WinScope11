...
[pscustomobject]@{
    Id                = 322
    Name              = 'TLS 1.0 Server Policy'
    Group             = 'TLS, Schannel & Crypto'
    GroupId           = 30
    Flags             = '[R][N]'
    Description       = 'Reports Schannel TLS 1.0 server protocol policy.'
    Adapter           = 'Native'
    Risk              = 'ReportOnly'
    ...
    Invoke            = {
        param($Context)
        $checks = @(
            @{Item='TLS 1.0 Server';Path='HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\TLS 1.0\Server';Names=@('Enabled','DisabledByDefault')}
        )
