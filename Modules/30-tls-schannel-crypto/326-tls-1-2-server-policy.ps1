...
[pscustomobject]@{
    Id                = 326
    Name              = 'TLS 1.2 Server Policy'
    Group             = 'TLS, Schannel & Crypto'
    GroupId           = 30
    Flags             = '[R][N]'
    Description       = 'Reports Schannel TLS 1.2 server protocol policy.'
    Adapter           = 'Native'
    Risk              = 'ReportOnly'
    ...
    Invoke            = {
        param($Context)
        $checks = @(
            @{Item='TLS 1.2 Server';Path='HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\TLS 1.2\Server';Names=@('Enabled','DisabledByDefault')}
        )
