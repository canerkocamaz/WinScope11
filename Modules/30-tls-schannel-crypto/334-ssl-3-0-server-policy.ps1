...
[pscustomobject]@{
    Id                = 334
    Name              = 'SSL 3.0 Server Policy'
    Group             = 'TLS, Schannel & Crypto'
    GroupId           = 30
    Flags             = '[R][N]'
    Description       = 'Reports Schannel SSL 3.0 server policy.'
    Adapter           = 'Native'
    Risk              = 'ReportOnly'
    ...
    Invoke            = {
        param($Context)
        $checks = @(
            @{Item='SSL 3.0 Server';Path='HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\SSL 3.0\Server';Names=@('Enabled','DisabledByDefault')}
        )
