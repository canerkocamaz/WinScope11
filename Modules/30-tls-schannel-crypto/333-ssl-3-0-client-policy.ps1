...
[pscustomobject]@{
    Id                = 333
    Name              = 'SSL 3.0 Client Policy'
    Group             = 'TLS, Schannel & Crypto'
    GroupId           = 30
    Flags             = '[R][N]'
    Description       = 'Reports Schannel SSL 3.0 client policy.'
    Adapter           = 'Native'
    Risk              = 'ReportOnly'
    ...
    Invoke            = {
        param($Context)
        $checks = @(
            @{Item='SSL 3.0 Client';Path='HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\SSL 3.0\Client';Names=@('Enabled','DisabledByDefault')}
        )
